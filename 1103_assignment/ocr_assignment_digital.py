...
image_path = os.path.join(base_dir, "data/digitalwriting.jpg")
pdf_path = os.path.join(base_dir, "data/digitalwriting.pdf")
truth_path = os.path.join(base_dir, "data/original.txt")   # 원문 텍스트 따로 저장된 파일
temp_dir = os.path.join(base_dir, "temp_digital")
result_dir = os.path.join(base_dir, "results_digital")
...
if __name__ == "__main__":
    ...
    plt.title("OCR Engine Accuracy Comparison (Digital Writing)", fontsize=14)
    ...
    graph_path = os.path.join(result_dir, "ocr_accuracy_digital.png")
    plt.tight_layout()
    plt.savefig(graph_path)

    result_txt = os.path.join(result_dir, "ocr_results_digital.txt")
    with open(result_txt, "w", encoding="utf-8") as f:
        f.write("📘 OCR 비교 결과 (Digital Writing)\n\n")
        f.write(f"[원본 텍스트]\n{original_text}\n\n")
